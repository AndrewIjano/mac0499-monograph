...
\section{$\gel$-MaxSAT solver}

- unsat in gel is a path init bot

(define cut)

- if we find a init-bot cut set in the graph and remove this edges, we have a satisfiable KB

- we do not want to remove every edge

- min cut! with infinite edges
...
\begin{algorithm}
	...
	\label{alg:max-sat}
	\begin{algorithmic}[1]
		\Function {$\gel$-Max-SAT-Solver}{$\tuple{\CC, \PP}, w$}
